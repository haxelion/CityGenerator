...
{
    int reponse = QMessageBox::information(this, "Fenetre d'aide", "<p> Bonjour à tous et bienvenue dans notre générateur de ville en 3D. Ce programme à pour but de generer une ville simpliste en 3D dans le zone a gauche de votre écran.<p> <p> Cliquez simplement sur ''démarrer'' pour lancer le programme. <p>  <p> Souhaitez-vous plus de détails ?", QMessageBox::Yes | QMessageBox::No);

    if (reponse == QMessageBox::Yes)
    {
        int reponse2 = QMessageBox::information(this, "Détails", "<p> Les différents paramètre que vous pouvez entrez sont sur la droite de votre écran. Ceux-ci obéissent parfois à certaines règles dépendantes de la génération de la ville et se modifient automatiquement lors de l'introduction de combinaison de paramètres impossibles pour que la ville soit générable.<p> <p> Pour faire bouger la ville, il suffit de cliquer avec votre souris sur l'écran et de la bouger en maintenant le bouton enfoncer, ou bien d'utiliser les flèches de votre clavier. <p> La case jardin permet de remplacer les batiments sans connexion avec un axe routier par des jardins. ", QMessageBox::Ok);
