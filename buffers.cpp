...
void Buffers::IBOGeneration(GLuint *IBO, int* indices)
{
    glGenBuffers(1, IBO);
...
void Buffers::generateElementBuffer(List<Block*> *BlockList)
{
    ...
    int *gardenIndices = new int [6*gardenNumber];
    float *buildingVertices = new float [16*5*buildingNumber];
    int *buildingIndices = new int [4*6*buildingNumber];
    float *roofVertices = new float [4*5*buildingNumber];
    int *roofIndices = new int [6*buildingNumber];
